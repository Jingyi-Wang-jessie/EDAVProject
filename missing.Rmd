...
We plot a bar to show each column’s missing value percentage in dataset. It is worth noticing that “OCCUPATION” has 4% missing values, because we would look into “OCCUPATION” in the following analysis. Other interested columns, “STATE”, “TRANSACTION_DT”, “TRANSACTION_AMT” have some values.
...
Addtionally, we applied visna to plot na values to further check if there is any missing pattern. We only kept columns used for later analysis and ignore columns, such as "MEMO_TEXT", "MEMO_CD" and etc. There is not any obvious missing patterns identified in the graph. 
```{r}
...
Specifically, we looked into “OCCUPATION” column. There are typos and the same occupation is written in different formats. For instance both “DIR” and "director" are in "OCCUPATION", but "DIR" is just abbreviation of "directo". Additionally, some rows have several occupations cluster in one cell indicating this contributor has multiple professional roles. For analysis simplicity, we only kept the first appeared title. Furthermore, we grouped occupations, for instance, “Retired-teachers” and “Retired-CEO” were clustered as “retired”. Unavoidably, this action introduced bias into the dataset.
